...
# 02) 역전파(back propageation) 

역전파 알고리즘이라는 것은 인공지능의 한 분야인 인공신경망을 학습시키기 위한 가장 기본적이고 일반적인 알고리즘이라고 할 수 있습니다.(계산적 편의성이 큽니다) 역전파라는 이름은 오차가 본래 진행방향과 반대 방향으로 전파 된다하여 붙여진 이름입니다.
